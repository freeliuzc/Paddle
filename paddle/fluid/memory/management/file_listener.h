...
class FileInfo {
 ...
 private:
  bool UpdateFileInfo(const struct stat& file_stat, unsigned char* cur_md5);
...
class FileListener {
 ...
 private:
  FileListener() : stop_running_(false), file_monitor_thread_(nullptr) {}

  ~FileListener() { StopMonitorThread(); }
